...
import { Filter, FilterRule } from '../decorators/filter-params.decorator';
import { Sort } from '../decorators/sort-params.decorator';

export const getOrder = (sort: Sort) => {
  return sort ? { [sort.property]: sort.direction } : {};
};

export const getWhere = (filter: Filter) => {
  if (!filter) return {};
